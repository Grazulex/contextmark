import { spawn } from 'node:child_process';
import { loadAgent } from '../../core/agent';
import { isLibraryInitialized } from '../../core/library';
import { colors, info } from '../../utils/colors';
import { LibraryNotFoundError } from '../../utils/errors';

export async function editAgentCommand(slug: string): Promise<void> {
  if (!isLibraryInitialized()) {
    throw new LibraryNotFoundError();
  }

  const agent = loadAgent(slug);
  const editor = process.env.EDITOR || process.env.VISUAL || 'nano';

  info(`Opening ${colors.brand(slug)} in ${colors.muted(editor)}...`);

  const child = spawn(editor, [agent.path], { stdio: 'inherit' });

  child.on('error', (err) => {
    console.error(`Failed to open editor: ${err.message}`);
    console.log(`  You can manually edit: ${agent.path}`);
  });

  child.on('close', (code) => {
    if (code === 0) {
      console.log();
      console.log(`${colors.success('✔')} Agent saved.`);
    }
  });
}
